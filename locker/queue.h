...
#define	STAILQ_NEXT(elm, field)	((elm)->field.stqe_next)struct {struct type *stqe_next;	/* next element */}
...
#define	STAILQ_INSERT_TAIL(head, elm, field) do {STAILQ_NEXT((elm), field) = NULL;STAILQ_LAST((head)) = (elm);(head)->stqh_last = &STAILQ_NEXT((elm), field);} while (0)
...
* List declarations.
*/
#define	BSD_LIST_HEAD(name, type) struct name {	struct type *lh_first;	/* first element */	}
...
#define	CIRCLEQ_FOREACH(var, head, field)for ((var) = CIRCLEQ_FIRST((head)); (var) != (void *)(head);(var) = CIRCLEQ_NEXT((var), field))

#define	CIRCLEQ_FOREACH_REVERSE(var, head, field)for ((var) = CIRCLEQ_LAST((head));(var) != (void *)(head);(var) = CIRCLEQ_PREV((var), field))
